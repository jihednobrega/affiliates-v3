...
export default function NewCampaignPage() {
  ...
  const handlePreviewScrollGradients = () => {
  ...
  }, [isPreviewOpen, selectedProductsData])
  ...
  const submitCampaign = async (data: CampaignFormValues) => {
    try {
      ...
      if (result.response?.success) {
        ...
        throw new Error(result.response?.message || 'Erro ao criar campanha')
      }
    } catch (error: any) {
      console.error('❌ Erro ao criar campanha:', error)
      toast({
        ...
        description:
          error.message ||
          'Ocorreu um erro ao criar a campanha. Tente novamente.',
        status: 'error',
        duration: 5000,
        isClosable: true,
      })
  ...
  return (
    <>
      <Head>
        <title>Nova Campanha | Affiliates</title>
      </Head>
      <AppLayout>
        <PageHeader>
            ...
            <HStack justify="space-between" mb={1} flex={1}>
              ...
              <HStack spacing={2}>
                <Button
                  ...
                  isDisabled={!watchedName || !watchedCommission}
                ...
                <Button
                  ...
                  loadingText={isUploading ? 'Fazendo upload...' : 'Criando...'}
                  isDisabled={
                    !watchedName ||
                    !watchedCommission ||
                    !watchedStartDate ||
                    ...
                    {selectedProductsData && selectedProductsData.length > 0 ? (
                      <Box position="relative" w="100%">
                        <Box
                          ...
                          cursor="grab"
                          userSelect="none"
                          onScroll={handlePreviewScrollGradients}
                          sx={{
                        ...
                        >
                          <HStack spacing={2} minW="max-content" maxW="100%">
                            {selectedProductsData.slice(0, 4).map((product) => (
                              <Box
                                key={product.id}
                                w="120px"
                                h="220px"
