...
export function ProductTable({ data }: ProductTableProps) {
    ...
    try {
      if (product.url) {
        ...
        toast({
          ...
          description: 'Este produto não tem mais um link criado.',
          status: 'warning',
          duration: 3000,
          isClosable: true,
        })
  ...
  const handleViewProduct = (product: RankingProduct) => {
    const productUrl = (product as any).destination_url || product.url
                ...
                <Td px={4} textColor="#131D53">
                  ...
                  >
                    <Button
                      ...
                      iconSpacing="6px"
                    >
                      {(item as any).linkData ? 'Copiar Link' : 'Sem Link'}
                    </Button>
                    <Button
