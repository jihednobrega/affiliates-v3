...
import { AccountantListMonthlyFee } from '@/services/types/accountant-billing.types'

interface MonthlyFeeTabProps {
  data: AccountantListMonthlyFee | null
  brandId: number
  month: string
...
export function MonthlyFeeTab({
  ...
  const handleGenerate = async () => {
    ...
    if (!data?.fee_amount) {
      toast({
        title: 'Erro',
        description: 'Valor da mensalidade não disponível',
        status: 'error',
        duration: 3000,
      })
      return
    ...
    try {
      await onGenerateInvoice(dueDate, data.fee_amount)
      toast({
    ...
    return (
      ...
      >
        <Text fontSize="sm" color="#131D5399">
          Nenhum dado de mensalidade disponível para esta marca
        </Text>
      </Flex>
    )
  }
    ...
    >
      <Flex align="center" gap={3}>
        <Flex
          ...
          p={1}
        >
          <DollarSign size={20} color="#203BE2" />
        </Flex>
      ...
      <VStack align="start" spacing={3}>
        ...
        <Box>
          ...
          <Text fontSize="2xl" color="#131D53" fontWeight="700">
            {(() => {
              const value = parseFloat(data.fee_amount)
